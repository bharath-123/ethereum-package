...
def generate_el_cl_genesis_data(
    ...
    genesis = plan.run_sh(
        ...
        run="cp /opt/values.env /config/values.env && ./entrypoint.sh all && mkdir /network-configs && mv /data/metadata/* /network-configs/ && mv /data/parsed /network-configs/parsed && python3 << 'PYEOF'\nimport yaml, json\ndef to_strings(obj):\n    if isinstance(obj, dict):\n        return {k: to_strings(v) for k, v in obj.items()}\n    elif isinstance(obj, list):\n        return [to_strings(item) for item in obj]\n    else:\n        return str(obj)\ndata = yaml.safe_load(open('/network-configs/config.yaml'))\njson.dump(to_strings(data), open('/network-configs/config.json', 'w'), indent=2)\nPYEOF",
        image=image,
        files=files,
        store=[
